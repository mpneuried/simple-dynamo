...
(function() {
  ...
  module.exports = DynamoTable = (function(_super) {
    ...
    function DynamoTable(table, options) {
      ...
      this.__defineGetter__("hashRangeDelimiter", function() {
        return "::";
      });
      ...
      if (this._isExistend(cb)) {
        ...
        this._attrs.validateAttributes(_create, attributes, function(err, attributes) {
          if (err) {
            ...
            if (_create) {
              return _this._create(attributes, function(err, _item) {
                var _obj, _ref;
                if (err) {
                  _this._error(cb, err);
                } else {
              ...
              return _this._update(_id, attributes, options, function(err, _curr, _old, _deletedKeys) {
                var _k, _new, _obj, _oldRem, _ref, _ref1, _v;
                if (err) {
                  _this._error(cb, err);
                } else {
                  if (_old) {
                    if (_this.isCombinedTable) {
                      _old[_this.hashKey] = _this._fixCombinedHash(_old[_this.hashKey]);
                    }
                    _obj = _this._dynamoItem2JSON(_curr, true);
                    _oldRem = {};
                    for (_k in _old) {
                      _v = _old[_k];
                    ...
                    if (_this.isCombinedTable) {
                      _curr[_this.hashKey] = _this._fixCombinedHash(_curr[_this.hashKey]);
                    }
                    _this.emit("update", _curr, _curr);
    ...
    DynamoTable.prototype.del = function(_id, cb) {
      ...
      if (this._isExistend(cb)) {
        query = this._deFixHash(_id);
        this._del(query, function(err, success) {
          if (err) {
            _this._error(cb, err);
          } else {
            _this.emit("delete", _id);
            cb(null, success);
          }
        });
      }
    ...
    DynamoTable.prototype.find = function() {
      ...
      if (this._isExistend(cb)) {
        ...
        if (this._isExistend(cb)) {
          ...
          _query.fetch(function(err, _items) {
            if (err) {
              _this._error(cb, err);
            } else {
    ...
    DynamoTable.prototype._update = function(id, attributes, options, cb) {
      ...
      this.get(id, function(err, current) {
        var item, _upd;
        if (err) {
          return cb(err);
        } else {
          item = _this.external.get(_this._deFixHash(id));
          _upd = item.update(_this._attrs.updateAttrsFn(current, attributes, options));
    ...
    DynamoTable.prototype._create = function(attributes, cb) {
      ...
      this._createId(attributes, function(attributes) {
        var _upd;
        _upd = _this.external.put(attributes);
        _upd = _this._checkSetOptions(_upd, attributes);
    ...
    DynamoTable.prototype._del = function(query, cb) {
      var _item,
        _this = this;
      _item = this.external.get(query);
      _item.destroy(function(err, success) {
        if (err) {
          cb(err);
        } else {
          cb(null, success);
    ...
    DynamoTable.prototype._fixHash = function(attrs) {
      ...
      if (this.isCombinedTable && (_attrs[_hName] != null)) {
        _attrs[this.hashKey] = this._fixCombinedHash(_attrs[this.hashKey]);
      }
      return _attrs;
    };
    ...
    DynamoTable.prototype._createId = function(attributes, cb) {
      ...
      this._createHashKey(attributes, function(attributes) {
        var _hName;
        if (_this.isCombinedTable) {
          _hName = _this.hashKey;
          attributes[_hName] = _this.name + _this.combinedHashDelimiter + attributes[_hName];
        }
        if (_this.hasRange) {
