(function() {
  ...
  Attributes = (function() {
    ...
    Attributes.prototype.updateAttrsFn = function(_current, _new, options) {
      var self;
      if (options == null) {
        options = {};
      }
      self = this;
      return function() {
        var val, _attr, _i, _k, _kc, _kn, _len, _ref, _tbl, _v;
        _tbl = self.table;
        ...
        for (_k in _new) {
          ...
          if (_k !== _tbl.hashKey) {
            _attr = self.get(_k);
            if ((_attr != null ? _attr.type : void 0) === "array" && !_.isArray(_new[_k])) {
              val = _new[_k];
              if (val["$add"] != null) {
                this.add(_k, val["$add"]);
              }
              if (val["$rem"] != null) {
                this.remove(_k, val["$rem"]);
              }
              if (val["$reset"] != null) {
                this.put(_k, val["$reset"]);
              }
            } else {
    ...
    Attributes.prototype.getQuery = function(table, query, cursor) {
      var isScan, _q, _ref;
